...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "my-v0-project:";
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkmy_v0_project"] = self["webpackChunkmy_v0_project"] || [];
...
// UNUSED EXPORTS: MessageConversation, RemotionRoot
...
const MessageBubble = ({ msg, appearSec, first, last, theme }) => {
  ...
  const progress = (0,esm.spring)({ frame: frame - appearFrame, fps, config: { damping: 12, mass: 0.9, stiffness: 170 } });
  const translateY = (0,esm.interpolate)(progress, [0, 1], [24, 0]);
  const scale = (0,esm.interpolate)(progress, [0, 1], [0.8, 1]);
...
const TypingBubble = ({ startSec, endSec, sent, theme }) => {
  ...
  const progress = (0,esm.spring)({ frame: local, fps, config: { damping: 14, mass: 0.8, stiffness: 140 } });
  const translateY = (0,esm.interpolate)(progress, [0, 1], [15, 0]);
  const dot = (i) => {
    const cycle = (local + i * 6) % 45;
    const scale = (0,esm.interpolate)(cycle, [0, 15, 30, 45], [0.4, 1, 0.4, 0.4]);
    return /* @__PURE__ */ react.createElement("div", { key: i, style: { width: 6, height: 6, borderRadius: 3, background: sent ? theme.colors.typingDotSent : theme.colors.typingDotReceived, transform: `scale(${scale})`, transition: "transform 0.15s linear" } });
  };
  return /* @__PURE__ */ react.createElement("div", { style: { display: "flex", justifyContent: sent ? "flex-end" : "flex-start", marginBottom: theme.bubble.marginBottom, transform: `translateY(${translateY}px)`, opacity: progress } }, /* @__PURE__ */ react.createElement("div", { style: { display: "flex", gap: 6, background: sent ? theme.colors.typingBubbleSent : theme.colors.typingBubbleReceived, padding: theme.bubble.padding, borderRadius: 18 } }, [0, 1, 2].map(dot)));
};
const Keyboard = ({ startSec, endSec, currentInputText, activeChar, theme }) => {
  const frame = (0,esm.useCurrentFrame)();
  const { fps } = (0,esm.useVideoConfig)();
  const startFrame = Math.round(startSec * fps);
  const endFrame = endSec != null ? Math.round(endSec * fps) : void 0;
  if (frame < startFrame) return null;
  if (endFrame !== void 0 && frame > endFrame) return null;
  const appearFrames = 12;
  const disappearFrames = 12;
  const sinceStart = frame - startFrame;
  const untilEnd = endFrame !== void 0 ? endFrame - frame : Infinity;
  const appearProgress = (0,esm.spring)({ frame: sinceStart, fps, config: { damping: 16, mass: 1, stiffness: 180 } });
  const slideIn = (0,esm.interpolate)(appearProgress, [0, 1], [KEYBOARD_HEIGHT, 0]);
  let translateY = slideIn;
  let opacity = appearProgress;
  if (endFrame !== void 0 && untilEnd <= disappearFrames) {
    const outProgress = 1 - untilEnd / disappearFrames;
    const eased = (0,esm.spring)({ frame: Math.round(outProgress * disappearFrames), fps, config: { damping: 18, mass: 0.9, stiffness: 140 } });
    const slideOut = (0,esm.interpolate)(eased, [0, 1], [0, KEYBOARD_HEIGHT]);
    translateY = slideOut;
    opacity = 1 - eased * 0.4;
  }
  ...
  const renderKey = (k) => {
    ...
    return /* @__PURE__ */ react.createElement("div", { key: k, style: baseStyle }, isSpace ? "" : k);
  };
...
const DeliveredBelow = ({ startSec, theme }) => {
  ...
  const startFrame = Math.round(startSec * fps);
  if (frame < startFrame) return null;
  const progress = (0,esm.spring)({ frame: frame - startFrame, fps, config: { damping: 20, stiffness: 170, mass: 0.8 } });
...
const MessageConversation = ({ messages, typingBeforeIndices, contactName, batteryLevel, theme: themeName = "imessage", alwaysShowKeyboard }) => {
  ...
  for (let i = 0; i < messages.length; i++) {
    const m = messages[i];
    const indicatorForced = typingBeforeIndices == null ? void 0 : typingBeforeIndices.includes(i);
    const autoIndicator = !m.sent && i > 0 && messages[i - 1].sent;
    const showIndicator = indicatorForced || autoIndicator;
    const base = i === 0 ? 0 : prevEnd + GAP_SECONDS;
    let typingIndicatorStart;
    let typingIndicatorEnd;
    let appearSec;
    let typingStart;
    let typingEnd;
  ...
  if (keyboardStart != null && currentSec >= keyboardStart && (!keyboardEnd || currentSec <= keyboardEnd)) {
    const active = entries.find((e) => e.msg.sent && e.typingStart !== void 0 && e.typingEnd !== void 0 && currentSec >= e.typingStart && currentSec < e.typingEnd);
    if (active && active.typingStart !== void 0) {
      ...
      const sending = entries.find((e) => e.msg.sent && e.typingEnd !== void 0 && currentSec >= e.typingEnd && currentSec < e.appearSec);
  ...
  if (keyboardStart != null) {
    ...
    if (frame >= ks) {
      if (keyboardEnd == null || frame < Math.round(keyboardEnd * fps) - KEYBOARD_DISAPPEAR_FRAMES) {
        ...
        const ke = Math.round(keyboardEnd * fps);
        const untilEnd = ke - frame;
        const outProgress = 1 - untilEnd / KEYBOARD_DISAPPEAR_FRAMES;
        const eased = (0,esm.spring)({ frame: Math.round(outProgress * KEYBOARD_DISAPPEAR_FRAMES), fps, config: { damping: 18, mass: 0.9, stiffness: 140 } });
        const slideOut = (0,esm.interpolate)(eased, [0, 1], [0, KEYBOARD_HEIGHT]);
  ...
  return /* @__PURE__ */ react.createElement(esm.AbsoluteFill, { style: { background: "transparent", fontFamily: theme.bubble.fontFamily, display: "flex", alignItems: "center", justifyContent: "center" } }, /* @__PURE__ */ react.createElement("div", { style: { width: 360, height: 780, background: theme.colors.background, borderRadius: 48, position: "relative", overflow: "hidden", boxShadow: "0 8px 24px rgba(0,0,0,0.25)", border: "0px solid transparent", isolation: "isolate" } }, /* @__PURE__ */ react.createElement(StatusBar, { batteryLevel, theme }), /* @__PURE__ */ react.createElement(NavigationHeader, { contactName, theme }), /* @__PURE__ */ react.createElement("div", { style: { position: "absolute", top: headerHeight + SAFE_TOP_INSET, left: 0, right: 0, bottom: 0, padding: "8px 12px 0", paddingBottom: Math.max(0, keyboardVisibleHeight - ACCESSORY_HEIGHT), display: "flex", flexDirection: "column", justifyContent: "flex-end", boxSizing: "border-box", zIndex: 100 } }, entries.map((entry) => {
    ...
    const next = (_b = entries[idx + 1]) == null ? void 0 : _b.msg;
    ...
    esm.Composition,
    {
      ...
      defaultProps: {
        ...
        messages: [
          ...
          { id: 8, text: "you do?! \u{1F60A}", sent: false, time: "0:14" }
        ]
      }
    }
  ));
