...
const CONFIG = {
  ...
  jobsCollection: process.env.APPWRITE_VIDEO_JOBS_COLLECTION_ID,
  rendersCollection: process.env.APPWRITE_VIDEO_RENDERS_COLLECTION_ID,
...
async function pollOnce() {
  try {
    ...
    if (!list.total) return false; // no jobs
    ...
    const compositions = await selectComposition({
      ...
      inputProps: normalizeConversation(conversation),
    });
    ...
    // Insert into renders collection (optional)
    if (CONFIG.rendersCollection && videoUrl && fileId) {
      ...
      } catch (e) { console.warn('⚠️ Failed to create video_renders doc:', e?.message || e); }
    }
...
async function main() {
  console.log('🛠️  Worker started. Polling for jobs...');
